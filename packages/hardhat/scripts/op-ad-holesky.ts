...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < imageUrls.length; i++) {
            ...
            // 让第一个广告由 owner 创建
            if (i === 0) {
                const tx = await dfToken.connect(owner).approve(adAlliance.getAddress(), budget);
                // wait for tx to be mined
                await tx.wait();

                const tx2 = await adAlliance.connect(owner).createAd(                    
                    imageUrls[i],
                    budget,
                ...
                // 其他广告由 addr1 创建
                const tx = await dfToken.connect(addr1).approve(adAlliance.getAddress(), budget);
                // wait for tx to be mined
                await tx.wait();

                const tx2 = await adAlliance.connect(addr1).createAd(
                    imageUrls[i],
                    budget,
                    parseEther("0.1") // Cost per click
                );
