...
async function main() {
    ...
    try {
        ...
        const tx = await dfToken.approve(await router.getAddress(), ethers.MaxUint256);
        // wait for tx to be mined
        await tx.wait();

        const tx2 = await token1.approve(await router.getAddress(), ethers.MaxUint256);
        // wait for tx to be mined
        await tx2.wait();

        const tx3 = await token2.approve(await router.getAddress(), ethers.MaxUint256);
        // wait for tx to be mined
        await tx3.wait();
        ...
        const tx7 = await router.swapExactETHForTokens(
            0, // 接受任何数量的代币
        ...
        const tx8 = await token1.approve(router.getAddress(), swapToken1Amount);
        // wait for tx to be mined
        await tx8.wait();
        
        const tx9 = await router.swapExactTokensForTokens(
            swapToken1Amount,
            0, // 接受任何数量的代币
