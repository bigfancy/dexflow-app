...
async function main() {
    ...
    try {
        ...
        // await englishAuction.connect(owner).bid(addresses.DFNFT, 1, bidAmount);
        ...
        // for (let i = 5; i < 8; i++) {
        //     const tx = await dfnft.connect(owner).approve(englishAuction.getAddress(), i);
        ...
        for (let i = 8; i < 10; i++) {
            const tx = await dfnft.connect(owner).approve(dutchAuction.getAddress(), i);
            // wait for tx to be mined
            await tx.wait();
